...
public class OrdersServices
{
    ...
    public async Task<IApiResponse> UpdateOrders(int orderId, UpdateOrderRequestDto request, int currentUserId, string? userRole = null)
    {
        try
        {
            ...
            var groupedOrders = orders
                ...
                .Select(g => new OrdersHistoriqueDto
                {
                    ...
                    DeletionWarning = g.Key == "Cancelled" ? 
                        "Les commandes annulées seront automatiquement supprimées après 72h" : null
                })
                .ToList();
        
            return new SuccessResponse<List<OrdersHistoriqueDto>>(groupedOrders, "Order history retrieved successfully");
        }
        catch (Exception ex)
        {
